"""Exact map-level SHAP analysis of the four physics feature maps.

Each of the four ensr input maps is treated as ONE feature: "present" = the
real map, "absent" = the zero baseline the conv head was zero-initialized
against.  With 4 features there are only 2^4 = 16 coalitions, so Shapley
values are computed EXACTLY by enumerating every coalition — no KernelSHAP
sampling.  The value function is the model's log-probability of the TRUE
source cell, v_n(S) = log p_theta(c*_n | x_n, maps in S), i.e. the training
objective; a Shapley value is therefore the map's average marginal
NLL-reduction in nats.
...
the 16 masked map stacks.

Models: DeepSets (primary — matches the Table II ablation), plus GNN and
Set Transformer when their retrained checkpoints exist.  Each checkpoint is
the deterministic retrain of the paper's seed-1 run (the DeepSets one is
verified region-identical to the frozen audit; see ../xai graphs/README.md).

Outputs (this folder):
  shap_map_contributions.csv   per-scenario Shapley value for each map
  shap_map_summary.{pdf,png,svg}   poster figure: mean |phi| bars + beeswarm

Sanity checks printed per model:
  - efficiency: max_n |sum_i phi_i - (v(all) - v(none))|  (~1e-12)
...
sys.path.insert(0, str(REPO / "Poster Graphics v2" / "xai graphs"))
...
from xai_style import (MAP_ORDER, MUTED, INK2, apply_style,    # noqa: E402
                       header, save_all)
...
N_MAPS = 4
N_COAL = 1 << N_MAPS
# model palette shared with the Poster Graphics v2 result figures
...
def load_test_view(cfg, dd):
    ...
    return pw.make_view(cfg, dd, ("test", d_clean), "noisy", "ensr")


def build_model(name, cfg, dd, device):
    """(model, base_cls, ckpt_path) or None if the checkpoint is missing."""
    if name == "DeepSets":
        path = dd / "checkpoints" / "pw_noisy_ensr_seed1.pt"
        cls, base = PhysHeadNet, DeepSetsT
        model = cls(cfg, N_MAPS)
    elif name == "GNN":
        from methane_t_gnn import GNNT
        path = dd / "checkpoints" / "pw_noisy_ensr_gnn_seed1.pt"
        base = GNNT
        model = pw.phys_head_on(GNNT, N_MAPS)(cfg)
    else:
        from methane_t_settransformer import SetTransformerT
        path = dd / "checkpoints" / "pw_noisy_ensr_st_seed1.pt"
...
def coalition_values(model, base_cls, d, stats, device, chunk=256):
    """V (N, 16): log p(true cell) under every coalition of present maps,
...
def exact_shapley(V):
    """Per-scenario exact Shapley values (N, 4) from the 16 coalition
...
def main():
    ...
    with open(csv_path, "w", newline="") as f:
        ...
        w.writerow(["model", "scenario_id", "n_masts"]
                   + [f"shapley_{c.lower().replace(' ', '_').replace('-', '_')}"
                      for c in cols]
    ...
    print("Table II's ablation adds the maps one at a time and finds the "
          "source-evidence map the largest single contributor (-21 m of "
          "-38 m total); the exact Shapley decomposition "
          f"{'AGREES — source evidence dominates' if order[0] == 'Source evidence' else 'DISAGREES with that ordering'} "
          "on the same seed-1 model and scenarios.")
    ...
    for y, (title, ch) in zip(ypos, MAP_ORDER):
        ...
        sc = ax.scatter(x, y + jit, c=ns, cmap=CMAP_COUNT, s=7,
    ...
    cb.set_label("masts", fontsize=13, color=INK2)
    ...
    header(fig, "Which physics map matters? Exact Shapley values over "
           ...
           "cell) · all 16 coalitions enumerated per scenario · 2,000 "
           "seed-1 test scenarios",
           ty=1.055, sy=1.016)
