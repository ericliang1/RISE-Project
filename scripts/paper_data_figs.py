...
def fig_components():
    """Measured wind, DeepSets base: each of the four physics images
    added cumulatively; the added map is shown beside each row."""
    ...
    CMO = LinearSegmentedColormap.from_list("o", ["#fcfcfb", "#fbe0d5",
        "#f6b899", "#eb6834", "#a03c14"])
    rows = [  # label, lo, hi, color, frac, thumbs
        ("input-only network", 107.9, 110.5, ORANGE, "0%", []),
        ("+ evidence image", 87.7, 88.5, "#86b6ef", "12%",
         [(det[0].reshape(64, 64), CMB)]),
        ("+ sensitivity image", 82.2, 84.2, "#5598e7", "15–17%",
         [(det[1].reshape(64, 64), CMB)]),
        ("+ wind-fragility image", 74.7, 76.3, "#2a78d6", "20–21%",
         [(ens[1].reshape(64, 64), CMO)]),
        ("+ fit-quality image", 70.1, 72.9, BLUE_D, "24–27%",
         [((10 - rm).reshape(64, 64), CMB)]),
    ]
    fig, ax = plt.subplots(figsize=(8.8, 5.0))
    fig.subplots_adjust(left=0.30, right=0.97, top=0.96, bottom=0.13)
    n = len(rows)
    bbx = dict(boxstyle="round,pad=0.2", fc=SURF, ec="none", alpha=0.95)
    for k, (lab, lo, hi, c, frac, thumbs) in enumerate(rows):
        ...
        ax.plot([lo, hi], [y, y], color=c, lw=6, solid_capstyle="round",
                zorder=3)
        ax.plot(mid, y, "o", color=c, ms=10, mec=SURF, mew=2, zorder=4)
        txt = (f"{lo:.0f}–{hi:.0f} m" if hi - lo >= 1
               else f"{mid:.0f} m")
        ax.annotate(f"{txt}    <50 m: {frac}",
                    xy=(hi + 2.5, y), va="center", fontsize=10,
                    color=INK2, bbox=bbx, zorder=6)
        if k > 0:
            pm = (rows[k - 1][1] + rows[k - 1][2]) / 2
            ax.annotate("", xy=(mid, y + 0.16), xytext=(pm, y + 0.84),
            ...
            ax.annotate(f"$-${pm - mid:.0f} m",
                        ...
                        color=INK, va="center", bbox=bbx, zorder=6)
        for j, (img, cm) in enumerate(thumbs):
            axk = fig.add_axes([0.015, 0.13 + 0.83 * (y + 0.16) / n,
                                0.052, 0.83 * 0.66 / n])
            ...
            for s in axk.spines.values():
                s.set_color(BASE)
    ax.axvline(50, color=MUTED, lw=1.4, ls=(0, (4, 3)), zorder=2)
    ax.annotate("50 m facility scale", xy=(50, n - 0.5), ha="center",
                fontsize=9.4, color=MUTED, bbox=bbx)
    ax.set_yticks(range(n))
    ax.set_yticklabels([r[0] for r in rows][::-1], fontsize=11)
    ax.set_xlim(42, 138); ax.set_ylim(-0.5, n - 0.3)
    ax.set_xlabel("median 90% region radius (m), measured wind, "
                  "3-seed range (DeepSets base)")
