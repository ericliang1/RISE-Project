"""Poster figures, third batch — the v2 set rebuilt on the super-emitter
benchmark (branch `super-emitter`): q ~ LogU(100, 500) kg/h, 4-8 masts,
final method `ens` = THREE physics channels (mean evidence z, wind spread,
mean log b over the same K=8 wind draws; the residual map was dropped and
the evidence channel now uses the asinh transform, never a hard clip).

  01_xai_feature_maps       what the physics head sees: the three input maps
  ...
  03_region_shapes_8masts   the actual conformal 90% regions for the
                            largest-contrast 8-mast scenario, per model
  04_radius_cdf             region-radius CDFs (paper Fig. 2 style)
  05_masts_curves           median radius vs mast count, 4-8
  ...
  01.5_xai_wind_ensemble    the eight per-wind-draw evidence maps behind the
                            ensemble channels, then consensus, disagreement,
                            and the final conformal region
...
Where a figure separates the three networks, the model palette is DeepSets
blue / GNN red / Set Transformer green (validated for CVD separation; every
curve is direct-labelled).

Every number is read from the frozen measured-wind conformal audits and the
frozen feature-map / posterior files under csr-data-se48, so the figures
agree with the branch results by construction.  Nothing is transcribed by
hand: the bootstrap CIs of Fig. 06 come from
`results/se48_table_deltas_ci.json`.

Usage: source scripts/env.sh && python "Poster Graphics v3/generate_poster_graphics_v3.py"
"""
import json
import pathlib
import warnings
...
DATA = pathlib.Path("/projectnb/rise-tower/eric1/csr-data-se48")
OUT = pathlib.Path(__file__).resolve().parent
REPO = OUT.parent
SITE = 500.0
N_CELLS = 64 * 64
SEEDS = (1, 2, 3)
MAPS_TAG = "ens"          # final method: 3 channels, residual map dropped
NETS = [("DeepSets", ""), ("GNN", "_gnn"), ("Set Transformer", "_st")]
MAST_LO, MAST_HI = 4, 8   # super-emitter benchmark population
EX_MASTS = 6              # example-scenario subgroup: midpoint of 4-8
...
def pick_scenario(d, n_masts=EX_MASTS):
    """Stated rule, continuing the v1/v2 batches: among held-out scenarios
    with exactly `n_masts` masts whose regions both cover the true source
    AND whose known-wind exact posterior is concentrated (max-cell mass
    >= 0.9 — true for the typical scenario; the share is printed), the one
    jointly closest (log scale) to that subgroup's median baseline and
    median physics-guided radii.  n_masts defaults to 6, the midpoint of
    the 4-8 benchmark population (v2 used 8, the midpoint of 4-12).  The
    concentration criterion is new in v3: on the super-emitter population
    a minority of scenarios keep a diffuse posterior even under the true
    wind, and the exact-Bayes reference panel should show the typical
    collapsed case, not an outlier."""
    zb, zo = audit("nomaps", 1), audit(MAPS_TAG, 1)
    rb, ro = rad(zb["sizes"]), rad(zo["sizes"])
    pmax = np.load(DATA / "ch4t_test_posterior.npz")["probs"].max(1)
    grp = d["n_sensors"].astype(int) == n_masts
    print(f"    known-wind posterior max-cell mass >= 0.9: "
          f"{(pmax >= 0.9).mean() * 100:.0f}% of all scenarios, "
          f"{(pmax[grp] >= 0.9).mean() * 100:.0f}% of the {n_masts}-mast "
          "subgroup")
    ok = grp & zb["covered"] & zo["covered"] & (pmax >= 0.9)
    dist = (np.log(rb / np.median(rb[grp])) ** 2
            + np.log(ro / np.median(ro[grp])) ** 2)
    return int(np.argmin(np.where(ok, dist, np.inf)))


# ---------------------------------- Fig 01: the three physics maps, one scene
def fig_feature_maps(d):
    """XAI gallery: the observed scene, the three physics-derived input maps
    (the full `ens` stack — the residual map is no longer part of the
    method), and the exact Bayes reference posterior, all for the same
    scenario."""
    i = pick_scenario(d)
    ...
    pmax = float(post.max())
    print(f"    exact-posterior max cell mass: {pmax:.4f}")
    ...
    panels = [
        ...
        ("E. Exact Bayes reference (known wind)", None, None,
         "with the true wind, the posterior concentrates\nonto essentially "
         "one grid cell — wind error is the whole game"),
        (None, None, None, None),   # legend / reading-guide panel
    ]

    fig, axes = plt.subplots(2, 3, figsize=(15.6, 11.4))
    for k, (ax, (title, img, cm, caption)) in enumerate(zip(axes.flat,
                                                            panels)):
        if title is None:
            # panel F: legend + how-to-read note, no axes
            ax.axis("off")
            h = [plt.Line2D([], [], color=INK, marker="o", lw=0, ms=11,
                            mec="white", mew=1.6,
                            label=f"sensor masts ({ns})"),
                 plt.Line2D([], [], color="#f2c230", marker="*", lw=0,
                            ms=19, mec=INK, mew=1.2,
                            label="true methane source")]
            ax.legend(handles=h, loc="upper left", frameon=False,
                      fontsize=15.5, bbox_to_anchor=(0.02, 0.96))
            ax.text(0.02, 0.60,
                    "The three maps (B–D) are the network's\nentire extra "
                    "input — closed-form physics,\ncomputed from measured "
                    "wind only,\naveraged over the same K = 8 wind draws.\n"
                    "\nThe residual (fit-quality) map of the\nearlier method "
                    "was dropped: the final\nmethod is these three channels.",
                    transform=ax.transAxes, fontsize=13.5, color=INK2,
        ...
        if img is not None:
            ...
            # the exact posterior concentrates in one cell (share printed on
            # the panel); drawn as geometry, since a heatmap of a near-delta
            # is blank
            ...
            ax.add_patch(Circle((cx + SITE / 128, cy + SITE / 128), 34,
                                ...
                                zorder=6))
            ax.annotate(f"{pmax * 100:.0f}% of the posterior:\none 7.8 m "
                        "cell", xy=(cx + 26, cy + 26), xytext=(215, 380),
    ...
    fig.text(0.075, 0.965, "the three closed-form input maps for one "
             f"held-out super-emitter scenario ({ns} masts, "
             f"{d['q'][i]:.0f} kg h$^{{-1}}$), computed from measured wind "
             "only; maps B–D are the network's extra input",
...
def fig_umap(d):
    ...
    trained — and neighbouring scenarios tend to share difficulty."""
    ...
    r_phys = rad(audit(MAPS_TAG, 1)["sizes"])

    # local difficulty structure: log-radius vs the 15-NN mean (embedding)
    logr = np.log(r_phys)
    nn = NearestNeighbors(n_neighbors=16).fit(emb)
    _, nbr = nn.kneighbors(emb)
    knn_mean = logr[nbr[:, 1:]].mean(1)
    rc = spearmanr(logr, knn_mean)
    print(f"    knn(15) difficulty rank corr: {rc.statistic:.3f} "
          f"(p = {rc.pvalue:.1e})")
    ...
    specs = [
        ...
        ("C. Achieved region radius", r_phys, CMG,
         ...
         f"neighbours tend to share difficulty (rank corr. "
         f"{rc.statistic:.2f})"),
    ]
...
# --------------------- Fig 01.5: the wind ensemble behind the input maps
def fig_wind_ensemble(d):
    ...
    and the branch's asinh zmap, and the recomputed mean is checked against
    the stored channel."""
    import sys
...
# ------------------------- Fig 03: actual guaranteed regions, 8 masts
def fig_areas_8masts(d):
    """The real conformal 90% regions for one 8-mast scenario (the densest
    network in the 4-8 benchmark), one panel per model: baseline region
    (orange) vs physics-guided region (blue).

    Scenario rule: among 8-mast scenarios covered by all six seed-1
    ...
    radius).  A best-case example, labelled as such."""
    ns = d["n_sensors"].astype(int)
    grp = ns == MAST_HI
    audits, contrast, ok = {}, np.zeros(len(ns)), grp.copy()
    for _, sfx in NETS:
        for tag in (f"nomaps{sfx}", f"{MAPS_TAG}{sfx}"):
            z = audit(tag, 1)
            r = rad(z["sizes"])
            audits[tag] = (z, r)
            ok &= z["covered"]
    ...
    for k, (ax, (lab, sfx)) in enumerate(zip(axes, NETS)):
        ...
        rw, ri = (audits[f"nomaps{sfx}"][1][i],
                  audits[f"{MAPS_TAG}{sfx}"][1][i])
    ...
    h = [plt.Line2D([], [], color=ORANGE, lw=2.6,
         ...
         plt.Line2D([], [], color=INK, marker="o", lw=0, ms=9, mec="white",
                    mew=1.5, label=f"sensor masts ({MAST_HI})")]
    fig.legend(handles=h, loc="lower center", ncol=5, frameon=False,
               fontsize=14.5, bbox_to_anchor=(0.5, -0.045))
    fig.suptitle("Eight masts: the guaranteed search region closes in on "
                 "the source", fontsize=21, fontweight="bold", x=0.055,
                 ha="left", y=1.125)
    fig.text(0.055, 1.035, "the held-out 8-mast scenario with the largest "
    ...
    with matplotlib.rc_context({"savefig.facecolor": "white"}):
        save(fig, "03_region_shapes_8masts")
    for lab, sfx in NETS:
        print(f"    {lab:16s} without {audits[f'nomaps{sfx}'][1][i]:5.1f}  "
              f"with {audits[f'{MAPS_TAG}{sfx}'][1][i]:5.1f}")
    return i


# ----------------------------------------- Fig 04: region-radius CDFs
def fig_cdf():
    """ECDF of region radius per model, solid = with maps, dashed =
    without, prespecified seed 1."""
    fig, ax = plt.subplots(figsize=(10.6, 6.8))
    curves = {}
    for lab, sfx in NETS:
        c = MODEL_C[lab]
        for tag, ls in ((f"{MAPS_TAG}{sfx}", "-"), (f"nomaps{sfx}", "--")):
    ...
    frac50 = np.mean([np.mean(rad(audit(f"{MAPS_TAG}{s}", 1)["sizes"]) < 50)
                      for _, s in NETS]) * 100
    frac50_no = np.mean([np.mean(rad(audit(f"nomaps{s}", 1)["sizes"]) < 50)
                         for _, s in NETS]) * 100
    ax.annotate(f"with maps, {frac50:.0f}% of scenarios\nare inside 50 m — "
                f"without maps,\n{frac50_no:.0f}%",
                xy=(23, 0.80), fontsize=15, color=INK, linespacing=1.45)

    # direct labels on each model's dashed (baseline) curve, at staggered
    # heights so the three families never collide
    for (lab, _), yq in zip(NETS, (0.28, 0.06, 0.45)):
        r, y = curves[(lab, "--")]
        x = float(np.interp(yq, y, r))
        ax.annotate(lab, xy=(x * 1.03, yq), fontsize=15, color=MODEL_C[lab],
                    fontweight="bold", ha="left", bbox=BOX, zorder=6)
    lo = min(r.min() for r, _ in curves.values()) * 0.92
    hi = max(r.max() for r, _ in curves.values()) * 1.05
    ax.set_xscale("log")
    ax.set_xlim(max(15, lo), min(420, hi))
    ticks = [t for t in (20, 50, 100, 200, 300, 400)
             if max(15, lo) <= t <= min(420, hi)]
    ax.set_xticks(ticks)
    ...
    ax.annotate("all 2,000 held-out super-emitter scenarios (4–8 masts, "
                "measured wind), prespecified seed 1;\nsolid left of dashed "
                "= improvement at equal coverage", xy=(0, -0.225),
...
def fig_masts(d):
    """Median radius vs number of masts (4-8), three seeds pooled, solid =
    with maps, dashed = without."""
    masts3 = np.tile(d["n_sensors"].astype(int), len(SEEDS))
    counts = np.arange(MAST_LO, MAST_HI + 1)
    fig, ax = plt.subplots(figsize=(10.6, 6.8))
    meds = {}
    for lab, sfx in NETS:
        c = MODEL_C[lab]
    ...
    ax.annotate("50 m facility-scale target",
                xy=(MAST_LO - 0.15, 51.8), ha="left", va="bottom",
                fontsize=14, color=MUTED)
    # direct labels, placed in measured clear space: DeepSets above its
    # (isolated) dashed curve; GNN below its solid curve near the 50 m
    # line; Set Transformer under its solid curve's start — the mid-plot
    # band where the six curves interleave has no unambiguous spot
    ds_y = float(np.interp(MAST_LO + 0.35, counts, meds[("DeepSets", "--")]))
    gnn_y = float(np.interp(MAST_LO + 2.35, counts, meds[("GNN", "-")]))
    st_y = meds[("Set Transformer", "-")][0]
    for lab, xy in (("DeepSets", (MAST_LO + 0.35, ds_y * 1.02)),
                    ("GNN", (MAST_LO + 2.35, gnn_y * 0.905)),
                    ("Set Transformer", (MAST_LO + 0.05, st_y * 0.94))):
        ax.annotate(lab, xy=xy, fontsize=15, color=MODEL_C[lab],
                    fontweight="bold", ha="left", bbox=BOX, zorder=6)
    ax.set_xlim(MAST_LO - 0.3, MAST_HI + 0.3)
    ax.set_xticks(counts)
    all_med = [v for m in meds.values() for v in m]
    ax.set_ylim(min(all_med) * 0.88, max(all_med) * 1.14)
    ...
    ax.annotate("median over the scenarios with each mast count (4–8 "
                "benchmark), three training seeds pooled, measured wind",
...
def fig_forest():
    ...
    radius and in the within-50 m share, with 95% paired-bootstrap CIs
    (results/se48_table_deltas_ci.json)."""
    ci = json.load(open(REPO / "results" / "se48_table_deltas_ci.json"))
    names = ["DeepSets", "GNN", "SetTransf"]
    labels = ["DeepSets", "GNN", "Set Transformer"]
    fig, axes = plt.subplots(1, 2, figsize=(13.6, 4.9), sharey=True)
    panels = [
        ("A. Change in region radius",
         lambda e: (e["d_radius_m"], *e["ci"]),
         "Δ radius (m) — negative is better"),
        ("B. Change in share within 50 m",
         lambda e: (e["d_frac50_pp"], *e["frac_ci"]),
         "Δ within-50 m (percentage points) — positive is better"),
    ]
    ys = np.arange(len(names))[::-1]
    for ax, (title, get, xlab) in zip(axes, panels):
        pts = []
        ...
        lo = min(min(pts) - 0.18 * span, -0.08 * span)
        hi = max(max(pts) + 0.18 * span, 0.08 * span)
    ...
    fig.text(0.045, 1.005, "same 2,000 seed-1 scenarios with and without "
             "maps; 95% CIs from paired-bootstrap resamples (Table I's "
             "effect columns) — every interval excludes zero",
             fontsize=13.5, color=MUTED, ha="left")
...
def fig_coverage():
    ...
    for off, kind, c, lab in ((-0.16, "nomaps", ORANGE,
                              ...
                              (0.16, MAPS_TAG, BLUE, "with physics maps")):
    ...
    ax.set_ylim(min(covs) - 0.006, max(covs) + 0.012)
    ...
    ax.set_title("Smaller regions are not bought with lost coverage",
                 fontsize=21, fontweight="bold", loc="left", pad=16)
    ax.annotate("one dot per training seed (three seeds per model and "
                "condition);\nsplit conformal holds every configuration "
...
if __name__ == "__main__":
    ...
    fig_wind_ensemble(d)
    print("figure 03 (areas at 8 masts)")
    fig_areas_8masts(d)
    print("figure 04 (cdf)")
    fig_cdf()
