"""Shared poster style for the v3 XAI figures — the single source of truth.

All v3 XAI scripts import everything visual from here so the figures cannot
drift apart: one rcParams block, one canonical feature-map order and naming,
one colormap per quantity, and one set of glyphs (white triangle = mast,
gold star = true source, red X = argmax).

v3 change vs v2: the method is `ens` — THREE physics channels (the residual
"Fit quality" map was dropped from the method), so MAP_ORDER has three
entries and every consumer adapts automatically.
"""
...
# Canonical feature-map presentation: display order -> ens channel index.
# 1) Source evidence  2) Sensor visibility  3) Wind-error sensitivity
# — used verbatim by every figure that shows the maps.  (The v2 "Fit
# quality" residual channel no longer exists in the method.)
MAP_ORDER = [("Source evidence", 0), ("Sensor visibility", 2),
             ("Wind-error sensitivity", 1)]
...
# feature-value gradient (SHAP beeswarm): house blue -> violet -> house red,
# read as low -> high map value
CMAP_FEATVAL = LinearSegmentedColormap.from_list(
    "featval", ["#2a78d6", "#8f6bb8", "#c94040"])
