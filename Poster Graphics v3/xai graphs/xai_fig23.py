"""XAI Figures 2 & 3 (v3) — embedding projection and integrated gradients,
from the real super-emitter benchmark checkpoint (see xai_fig1.py).
...
coloured by achieved 90% region radius and by mast count.  The structure
metrics (silhouette of a radius split, Spearman of axis 1 vs masts / log
radius, and the within-mast-count correlation) are computed, printed, and
...
w.r.t. the three input feature maps for the Figure-1 scenario (read from
fig1_scenario.json).  Baseline = zero maps, 50 midpoint steps; the raw
sensor tokens are held fixed, so this attributes the maps pathway.  The
completeness identity (sum of IG = logp(full) - logp(baseline)) is
printed.  Panels follow the canonical map order shared with Figure 1 via
xai_style.MAP_ORDER.

Usage (from repo root, after xai_fig1.py):
...
N_MAPS = len(MAP_ORDER)          # 3: the ens stack
...
def fig1_scenario():
    return int(json.load(open(HERE / "fig1_scenario.json"))["index"])
...
def load_model_and_views():
    ...
    ckpt = torch.load(dd / "checkpoints" / "pw_noisy_ens_seed1.pt",
...
def fig2(d_tst, st_tst, emb, radii):
    ...
    for name, pr in (("UMAP", proj_umap), ("PCA", proj_pca)):
        ...
        strat = np.mean([spearmanr(pr[masts == k, 0],
                         ...
                         for k in range(MAST_LO, MAST_HI + 1)
                         if (masts == k).sum() > 30])
    ...
    for name, pr in (("umap", proj_umap), ("pca", proj_pca)):
        ...
        # title states whichever structure the measured correlations
        # actually support (v2's model organized by mast count; the v3
        # head-warmup model organizes by difficulty)
        if abs(strat) > abs(rm):
            title = ("The embedding organizes by difficulty — even "
                     "within a mast count")
        else:
            title = ("The embedding tracks mast count — an input — "
                     "not difficulty")
        header(fig, title,
               f"{name.upper()} of the decoder-input embedding, DeepSets "
               f"ens seed 1, 2,000 scenarios · silhouette {sil:.2f} · "
...
def fig3(cfg, device, model, d_tst, st_tst, Pt, radii, steps=50):
    ...
    s_full = st_tst[i].to(device).view(1, N_MAPS, N_GRID, N_GRID).float()
    alphas = ((torch.arange(steps, device=device) + 0.5) / steps)
    stats = (alphas[:, None, None, None, None] * s_full).view(
        steps, N_MAPS, N_GRID, N_GRID).requires_grad_(True)
    ...
    # symmetric colour limits: 99th percentile of |IG| across the maps
    vmax = float(np.percentile(np.abs(ig), 99))
    ext = [0, SITE, 0, SITE]
    fig, axes = plt.subplots(1, N_MAPS, figsize=(12.8, 5.4))
    ...
    header(fig, "Which map drove the decision? Integrated gradients",
           f"Figure-1 scenario {sid} ({ns} masts) · red = "
           "toward the predicted cell, blue = away · zero-map baseline, "
           f"{steps} steps · completeness |error| "
