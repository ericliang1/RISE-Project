...
cost. Single-seed channel diagnostics locate the gain: under exact
wind the evidence image alone reaches 65.0\,m, the sensitivity image
alone 98.5\,m, and both together 47\,m, so the correlation computation
is the operative ingredient. At the other extreme, the classical score
alone
...
3 seeds; $\dagger$: random-image negative control, 1 seed, diagnostic).
Each image type is added on top of the previous ones.}
\label{tab:comp}
\setlength{\tabcolsep}{4pt}
\begin{tabular}{lccc}
\toprule
...
+ evidence pair ($\bar z_c$, $\overline{\log b_c}$) & 0.91--0.92 &
82.2--84.2 & 15--17\% \\
...
of the same shape and scale ($\dagger$ in Table~\ref{tab:comp})
matches
