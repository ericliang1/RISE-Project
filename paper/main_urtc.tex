...
\begin{figure}[t]
\centering
\includegraphics[width=0.74\linewidth]{../figures/paper/fig_data_example.pdf}
\caption{Representative exact-wind scenario: 90\% regions of the
labels-only baseline and of our method (conformal masks), the
exact-model reference (equal-area circle), the sensor masts, and the
50\,m facility scale. Selection rule: the scenario jointly closest (log
scale) to the median baseline radius and the median method radius, i.e.,
typical difficulty with typical improvement, not a best case.}
\label{fig:circles}
\end{figure}
...
(subgroup diagnostics appear in the appendix). Calibration is performed
...
exact-model reference region; 2{,}000 test scenarios.}
\label{tab:audit}
\begin{tabular}{lcc}
\toprule
Method & Coverage & Radius (m) \\
\midrule
Exact-model reference region & 0.89--0.95 & \textbf{6.2--7.0} \\
...
reveals that one of them delivers regions more than ten times larger
than the exact-model reference (Table~\ref{tab:audit}). Two input images
and a small head close half of the distance. Relative to the labels-only
...
overconfidence with full-site regions. Channel ablations confirm the
correlation computation is the operative ingredient: the evidence image
alone reaches 65.0\,m, the sensitivity image alone 98.5\,m, both
channels 47\,m.

\begin{table*}[t]
\centering
\caption{Left: subgroup diagnostics (prespecified groups; seed-1 runs; diagnostic only, the conformal guarantee remains marginal over the complete test distribution). Right: reproducibility summary; full configuration and code in the repository.}
\label{tab:subgroup}
\footnotesize
...
\midrule
Split & 10k/1k/2k/2k & Optimizer & AdamW, cosine lr, wd $10^{-4}$ \\
Site, grid & 500\,m; $64^2$ (7.8\,m cells) & Batch, epochs & 256; 200, best val-NLL state \\
Masts & 4--12, uniform & Precision & bfloat16 autocast, clip 1.0 \\
Readings & 30 $\times$ 1-min; drop $\le$20\% & Targets & smoothed 0.75 cells; D4 \\
Noise $\sigma$ & log-unif.\ 0.2--3\,ppm & $z$, $\log b$ norm.\ & $\pm$60, $\div$10; $(\log_{10}b{+}8)/6$ \\
Rate & log-unif.\ 10--500\,kg\,h$^{-1}$ & Residual norm.\ & min-shift, median-scale, clip 10 \\
Wind & 1--8\,m\,s$^{-1}$; AR(1) $\rho{=}0.7$ & Parameters & 4.88\,M; head 10{,}721 (0.22\%) \\
Meander & 20--40$^\circ$, 10--30\% & Seeds & fixed root; train 1--3; score seed \\
Stability & PG A--F, uniform & Hardware & NVIDIA L40S \\
Wind error & $10^\circ$, 10\% per step & Timing (b=1) & 30.4\,ms med., 31.7\,ms p95 \\
\bottomrule
\end{tabular}
\end{minipage}
\end{table*}
...
% dagger: diagnostic negative control, 1 seed
\begin{tabular}{lccc}
\toprule
...
~~random images, same head$^{\dagger}$ & 0.89 & 107.5 & 0\% \\
...
\begin{figure}[t]
\centering
\includegraphics[width=0.93\linewidth]{../figures/paper/fig_data_ecdf.pdf}
\caption{Region-radius distributions over all 2{,}000 test scenarios
(dashed: exact wind; solid: measured wind). The curves give medians,
tails, and the fraction below the 50\,m line in one view; our
measured-wind distribution dominates the baseline's exact-wind
distribution over most of its range.}
\label{fig:ladder}
\end{figure}
...
50\,m (p90\,$=$\,131\,m); subgroup diagnostics
(Table~\ref{tab:subgroup}) show no collapsing group but mild
under-coverage for sparse-mast scenarios (0.865--0.867 at 4--6 masts), a
concrete target for group-conditional calibration. Field validation on
