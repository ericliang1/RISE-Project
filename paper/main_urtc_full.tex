...
masks), with the 50\,m facility scale. Selection rule: among scenarios
with $\ge$10 masts whose region meets the 50\,m scale, the one at that
subset's median radius, i.e., representative of the quarter of cases
...
The fix is architecture-independent: the same images move a GNN from
56--57\,m to 46.2--47.5\,m and a Set Transformer from 69--71\,m to
46.2--47.3\,m, so all nine runs of three network families land inside
a 1.5\,m band, and the gain is available to whatever architecture a
practitioner already has. The classical score alone, calibrated with no
network, identifies where the evidence lies but not how much to trust
it, and pays for that overconfidence with full-site regions. All
...
\begin{figure}[t]
\centering
\includegraphics[width=0.96\linewidth]{../figures/paper/fig_data_dist.pdf}
\caption{Region-radius distributions over all 2{,}000 test scenarios
under measured wind. The annotated points show how the headline
statistics coexist: 27\% of scenarios fall below the 50\,m facility
scale while the median sits at 71\,m; the no-images distribution never
reaches the scale.}
\label{fig:dist}
\end{figure}
