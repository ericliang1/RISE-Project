...
Measured at batch size one over test scenarios after warm-up,
transfers included: on an NVIDIA L40S the pipeline takes 30.4\,ms
median end to end (preprocessing 29.6, forward pass 0.8); on a single
commodity CPU process it takes 0.49\,s median (0.95\,s p95). Since one
scenario consumes a 30-minute observation window, even CPU-only
deployment leaves more than three orders of magnitude of headroom, and
no site GPU is required. We verified \eqref{eq:resid} against direct
...
Wind error & $10^\circ$ dir., 10\% speed, per step & Timing (batch 1) & GPU 30.4\,ms med; CPU 0.49\,s med \\
\bottomrule
\end{tabular}
\end{table}
