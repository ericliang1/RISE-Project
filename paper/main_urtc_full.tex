...
\begin{figure}[t]
\centering
\includegraphics[width=0.54\linewidth]{../figures/paper/fig_data_example.pdf}
...
same images help every architecture and why the gains grow as the
images carry more of the wind-error structure.
...
\begin{table}[t]
\centering
\caption{Cumulative image ablation on the DeepSets base (exact wind,
where the four images provably reduce to the evidence--sensitivity
pair, Eq.~\ref{eq:reduction}); $^{1s}$: single seed. The sensitivity
image without the evidence image leaves the baseline unchanged
(98.5\,m$^{1s}$), so the correlation computation is the operative
ingredient.}
\label{tab:comp}
...
\midrule
none (input-only network) & 0.89 & 94--98 & 0\% \\
+ evidence image ($z_c$) & 0.88 & 65.0$^{1s}$ & 33\% \\
\textbf{+ sensitivity image ($\log b_c$)} & 0.89 &
\textbf{47.1--47.7} & \textbf{52--53\%} \\
\bottomrule
\end{tabular}
\end{table}
...
cost. Table~\ref{tab:comp} ablates the images cumulatively: the
evidence image alone recovers most of the gain and the sensitivity
image completes it. At the other extreme, the classical score alone
identifies where the evidence lies but not how much to trust it, and
pays for that overconfidence with full-site regions.
...
guaranteed regions become: from 82--84\,m using
