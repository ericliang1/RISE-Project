...
\begin{figure}[t]
\centering
\includegraphics[width=0.37\linewidth]{../figures/paper/fig_data_example.pdf}
...
($10^\circ$/10\% per-step anemometer error; radii in m; 3 seeds per
cell. For reference, direct physics inversion retains its guarantee
only at coverage 1.00, returning the full site, 282\,m.)}
\label{tab:wind}
\setlength{\tabcolsep}{2.2pt}
\footnotesize
\begin{tabular}{lcccccc}
\toprule
 & \multicolumn{3}{c}{Without images} &
\multicolumn{3}{c}{With images} \\
Base network & Cov. & Radius & $<$50\,m & Cov. & Radius &
$<$50\,m \\
\midrule
DeepSets & 0.90--0.91 & 108--110 & 0\% & 0.90--0.92 &
\textbf{70--73} & 24--27\% \\
GNN & 0.90--0.91 & 69--71 & 1--5\% & 0.91 &
\textbf{68--69} & 27--29\% \\
Set Transformer & 0.89 & 80--82 & 7--12\% & 0.91 &
\textbf{69--70} & 26\% \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.64\linewidth]{../figures/paper/fig_data_masts.pdf}
\caption{Sensor-count ablation for all three base networks (measured
wind, seed-1 diagnostic; $\approx$210--240 test scenarios per mast
count; solid: with images, dashed: without). With the images, the three
networks track a single curve that approaches the facility scale as
masts are added; the conformal guarantee remains marginal over the full
distribution.}
\label{tab:masts}
\end{figure}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.64\linewidth]{../figures/paper/fig_data_dist.pdf}
\caption{Region-radius distributions over all 2{,}000 test scenarios
under measured wind, for all three base networks (seed 1; solid: with
images, dashed: without). The three with-images distributions are
nearly indistinguishable: the physics images, not the architecture, set
the achievable precision; about a quarter of regions fall below the
50\,m scale.}
\label{fig:dist}
\end{figure}
...
held-out data, does not change this. A negative control isolates the cause: four \emph{random} images
through the identical head (one seed, diagnostic) match the no-physics
baseline (107.5\,m), so the improvement traces to the physics content
of the images, not added capacity. The learned recipe behaves differently, and the pattern
...
simulation-based inference,'' AISTATS, 2021.
