...
\begin{figure*}[t]
\centering
\includegraphics[width=0.95\textwidth]{../figures/paper/fig_pipeline.pdf}
\caption{The pipeline. The physics input processor turns raw sensor
data and the wind sensor's stated error budget into four per-cell
images (Eqs.~\ref{eq:zmap}--\ref{eq:resid}), which any set network
reads through a small zero-initialized head joined at the logits
(Eq.~\ref{eq:model}); split conformal calibration certifies the 90\%
region under the deployment wind condition (Eq.~\ref{eq:region}). At
$\varepsilon{=}0$ the images provably reduce to the classical
matched-filter pair (Eq.~\ref{eq:reduction}).}
\label{fig:pipeline}
\end{figure*}

\begin{figure}[t]
\centering
\includegraphics[width=0.7\linewidth]{../figures/paper/fig_data_example.pdf}
\caption{Typical well-instrumented scenario: 90\% regions of the
...
50\,m facility scale. Selection rule: within the 10--12-mast subgroup,
the scenario jointly closest (log scale) to that subgroup's median
baseline and median method radii, i.e., typical for well-instrumented
sites, not a best case.}
\label{fig:circles}
\end{figure}
...
not a matter of expressiveness; the images improve it as well
(non-overlapping three-seed ranges), so the recipe composes with other
estimators rather than competing with them.
